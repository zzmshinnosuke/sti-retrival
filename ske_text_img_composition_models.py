...
class SkeTextImgCompositionBase(torch.nn.Module):

    def __init__(self):
        super(SkeTextImgCompositionBase,self).__init__()
    ...
    def extract_skeImg_feature(self,skes):
        raise NotImplementedError

    def extract_skeVector_feature(self,skes):
        raise NotImplementedError
    ...
    def compute_loss(self,skes_img,texts,imgs_target,soft_triplet_loss=True):
        # print("Test print ",skes_img)
        ske_img=self.compose_img_text(skes_img,texts)
        ...
        assert (ske_img.shape[0] == img2.shape[0] and
                ske_img.shape[1] == img2.shape[1])

        if soft_triplet_loss:
            return self.compute_soft_triplet_loss_(ske_img,img2)
        else:
            return self.compute_batch_based_classification_loss_(ske_img,img2)
...
class EncoderBase(SkeTextImgCompositionBase):
    def __init__(self,texts,embed_dim,img_model):
        ...
        if img_model=="resnet18":  
            ...
            model.fc=torch.nn.Sequential(torch.nn.Linear(model.fc.in_features,embed_dim))
        elif img_model=="inceptionv3":     
            model= torchvision.models.inception_v3(pretrained=True)
            num_ftrs = model.AuxLogits.fc.in_features
            model.AuxLogits.fc = torch.nn.Linear(num_ftrs, embed_dim)
            model.aux_logits=False
            # Handle the primary net
            num_ftrs = model.fc.in_features
            model.fc = torch.nn.Linear(num_ftrs,embed_dim)
        else :
            print('Invalid imgmodel',opt.dataset)
            sys.exit()
    ...
    def extract_skeImg_feature(self,skes):
        return self.img_model(skes)



class SkeOnlyModel(EncoderBase):
    def compose_img_text(self,imgs,texts):
        return self.extract_skeImg_feature(imgs)

class ImgOnlyModel(EncoderBase):
    def compose_img_text(self,imgs,texts):
        return self.extract_skeImg_feature(imgs)

class TextOnlyMode(EncoderBase):
    def compose_img_text(self,imgs,texts):
        return self.extract_text_feature(texts)

class Concat(EncoderBase):
...
class TIRGLastConv(EncoderBase):
